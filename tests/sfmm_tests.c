...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	...
	assert_quick_list_block_count(0, 0);
	assert_free_block_count(0, 1);
	assert_free_block_count(656, 1);

	cr_assert(sf_errno == 0, "sf_errno is incorrect!");
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	...
	cr_assert(sf_errno == 0, "sf_errno is incorrecto!");
...
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
	...
	sf_errno = 0;
	size_t size1 = 60, size2 = 200;
	void *f = sf_malloc(size1);
	void *n = sf_malloc(size2);
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
	...
	sf_errno = 0;
	size_t size1 = 60, size2 = 200, size4 = 50;
	void *a = sf_malloc(size1);
	void *b = sf_malloc(size2);
	void *d = sf_malloc(size4);
...
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
	...
	sf_errno = 0;
	size_t size1 = 60, size2 = 200, size4 = 50, size5 = 90;
	void *z = sf_malloc(size1);
	void *x = sf_malloc(size2);
	void *y = sf_malloc(size4);
	void *c = sf_malloc(size5);
